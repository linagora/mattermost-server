...
func (a *App) authenticateUser(user *model.User, password, mfaToken string) (*model.User, *model.AppError) {
	...
	if user.AuthService == model.USER_AUTH_SERVICE_LDAP {
		if !ldapAvailable {
			...
			return user, err
		} else if ldapUser, err := a.checkLdapAuthentification(user, password); err != nil {
			err.StatusCode = http.StatusUnauthorized
			return user, err
		} else {
...
func (a *App) checkLdapAuthentification(user *model.User, password string) (*model.User, *model.AppError) {
	...
	if err := conn.Bind("uid="+user.Username+","+*a.Config().LdapSettings.BaseDN, password); err != nil {
		errBind := model.NewAppError("checkUserPassword", "api.user.login.invalid_credentials", nil, "user_id="+user.Id, http.StatusUnauthorized)
		return nil, errBind
	} else {
		if errCheck := checkUserNotDisabled(user); errCheck != nil {
			errCheck = model.NewAppError("login", "api.user.login_ldap.not_available.app_error", nil, "", http.StatusNotFound)
			return nil, errCheck
		}

		if result := <-a.Srv.Store.User().GetByEmail(user.Email); result.Err != nil {
			UserCreated, errCreation := a.createUser(user)
			if errCreation != nil {
